...
class MilestoneControllerTest {
    ...
    void createMilestone() throws Exception {
        ...
        MilestoneCreatRequest mileStoneCreatRequest = new MilestoneCreatRequest(title, description, dueDate);

        // when,then
        mockMvc.perform(post("/api/" + organizationTitle + "/milestones")
                        .contentType(MediaType.APPLICATION_JSON)
                        .content(objectMapper.writeValueAsString(mileStoneCreatRequest))
                )
                .andDo(print())
